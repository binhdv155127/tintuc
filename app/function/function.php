...
function changeTitle($str,$strSymbol='-',$case=MB_CASE_LOWER){// MB_CASE_UPPER / MB_CASE_TITLE / MB_CASE_LOWER
	$str=trim($str); // hàm trim sẽ loại bỏ các khoảng trắng ở đầu và đuôi chuỗi
	if ($str=="") return "";
	$str =str_replace('"','',$str);  /// thay thế '''' thành ''
	$str =str_replace("'",'',$str);
	$str = stripUnicode($str);  // hàm bỏ dấu tiếng việt đã viết bên dưới
	$str = mb_convert_case($str,$case,'utf-8');  // chuyển hoa thnahf chữa thường
